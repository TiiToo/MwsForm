...
use Sistema\MWSFORMBundle\Form\DataTransformer\EntityToJsonOneTransformerArticulo;
 ...
 */
class Select2articuloType extends AbstractType
{
    /**
     * @var ObjectManager
     */
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        $dataConnect = array(
            'class' => $options['class'],
            'om' => $this->om,
            'cantidadMaxima' => $options['configs']['cantidadMaxima'],
            'valCantidadMaxima' => $options['configs']['valCantidadMaxima'],
        );
        if ($options['configs']['multiple'] === true) {
            $transformer = new EntityToJsonTransformer($dataConnect);
        } else {
            $transformer = new EntityToJsonOneTransformerArticulo($dataConnect);
        }
        $builder
